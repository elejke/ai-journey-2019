...
def solver_5(task):
    ...
    def cast_to_form(word_to_cast_morph_form, src_morph_form):
        res = word_to_cast_morph_form.inflect(src_morph_form.tag.grammemes)
        if res is not None:
            return res.word
        res = word_to_cast_morph_form.inflect({src_morph_form.tag.POS,
                                               src_morph_form.tag.gender,
                                               src_morph_form.tag.number,
                                               src_morph_form.tag.case} - {None})
        if res is not None:
            return res.word
        return word_to_cast_morph_form.word
    ...
    for sent in text.split("\n")[1:]:
        ...
        if match is not None:
            ...
            morph_form = morph.parse(words[-1])[0]
            normalized_words.append(morph_form.normal_form)

            par = copy.deepcopy(paronyms.get(normalized_words[-1], []))
            for i in range(len(par)):
                par[i] = cast_to_form(morph.parse(par[i])[0], morph_form)
    ...
    return word_paronyms[max_dist_indices[0]][max_dist_indices[1]]
